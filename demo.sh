...
DEFAULT_VEHICLE_NAME="Fleetwise_Vehicle"
VEHICLE_NAME="Vehicle_DEMO"
...
CLI_VERSION=`This from Line 122: AWS: aws --version`
echo ${CLI_VERSION}
...
NODE_LIST=`( echo ${DBC_NODES} | jq -r .[].sensor.fullyQualifiedName | grep Vehicle\\. ) | jq -Rn [inputs]`
...
:'
sleep 2
echo "This from Line 520: Suspending campaign"
aws iotfleetwise update-campaign \
    ${ENDPOINT_URL_OPTION} --region ${REGION} \
    --name ${NAME}-campaign \
